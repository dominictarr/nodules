...
//create the functions for useLocal,usePackage,getModuleSource,getCachePath depending on type of engine??
	...
	//only used by getModuleSource
	
		//drop .js if it's present.
		if(id.substring(id.length - 3) == ".js"){
			id = id.substring(0, id.length - 3);
		}
		var uri = resolveUri("", id, enginePackage.mappings);//looks in enginePackage.mappings
...
/*
	returns value of packages hash, where is packages[] updated?
see load: exports.load = function(uri, require)

*/
function getPackage(uri){
	return packages[getPackageUri(uri)] || {mappings: packages[""].mappings};
}
//where is package set?0
function makeWorker(Constructor, currentId){
	...
	/*
	creates a new require function which is aware of the id of the current module.
	it uses this to look up the mappings defined for that package and load the dependencies (?)
	
	I think it decides when it calls resolveUri. 
		passes curentId, the new id, getPackage(currentId.mappings)
		presumably, this selects the Uri (which may be a url) from the current id, and the mappings.
	
	*/
function makeRequire(currentId){
	var require = function(id){
		var uri = resolveUri(currentId, id, getPackage(currentId).mappings);//get uri from the package, and resolve it to relative?
...
exports.load = function(uri, require){
	//loads a module, presumably.
	//called in ensure from makeRequire()
	//uri is the path to the module,
	//require is from makeRequire
	
	//

	//get correct uri from protocols.
	//uris should start with file: http: jar:, zip:, or data:
